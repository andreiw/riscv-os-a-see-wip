...
|===
| ID#     ^| Requirement
| `UEFI_010` | The firmware MUST be a 64-bit 64-bit UEFI implementation.
| `UEFI_020` | The 3rd Party UEFI Certificate Authority (CA) requirements on UEFI memory mitigations cite:[MSUefiCaRequirements] MUST be met.
| `UEFI_030` a| BRS-I specific memory map requirements:

              * The default memory space attribute MUST be `EFI_MEMORY_WB`.
              * Address translation MUST be enabled.
              * MUST only use `EfiRuntimeServicesData` memory type for describing any SMBIOS data structures.
...
| `UEFI_060` | Declaring the `EFI_CONFORMANCE_PROFILES_UEFI_SPEC_GUID` conformance profile.
...
| `UEFI_070` | Declaring the `EFI_CONFORMANCE_PROFILE_BRS_1_0_SPEC_GUID` conformance profile `({ 0x05453310, 0x0545, 0x0545, { 0x05, 0x45, 0x33, 0x05, 0x45, 0x33, 0x05, 0x45 }})`.
